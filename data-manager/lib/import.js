...
export const importCollection = async (collectionName, filePath, options = {}) => {
    ...
    fs.createReadStream(filePath)
        ...
        .on('end', async () => {
            ...
            const affectedConstructorNames = new Set(); // New tracking for Names (from Des/Mod)
            ...
            for (let i = 0; i < rows.length; i++) {
                ...
                try {
                    ...
                    // Link ID by Name if missing (Legacy/Fallback, now Adapter handles ID gen, but safeguard linked ID)
                    // The Adapter `adaptDesarrollo` generates deterministic ID `slug(cons-nom)`.
                    // So we mostly rely on that. But if ID is empty implies name/const missing?
                    // Safe verification:
                    ...
                    // ID Handling & Merge Safety Logic
                    let docRef;
                    // For Developers, we might have generated an ID in the adapter (slug) or have one from CSV
                    if (finalData.id) {
                    ...
                    // Developer & Development Specific Logic: "Safe Merge" & Initialization
                    if (collectionName === 'desarrolladores') {
                        try {
                            const docSnap = await docRef.get();
                            if (!docSnap.exists) {
                                // NEW DOCUMENT: Initialize Operational Fields
                                finalData.operacion = { asesoresAutorizados: [], asesoresConLeads: [] };
                                finalData.stats = { ofertaTotal: 0, viviendasxVender: 0 };
                            } else {
                                // EXISTING: Protect operacion
                                delete finalData.operacion;
                                delete finalData.stats;
                            }
                        } catch (e) { logger.error(`Error Upsert Dev ${finalData.id}`, e); }

                    } else if (collectionName === 'desarrollos') {
                        try {
                            const docSnap = await docRef.get();
                            if (!docSnap.exists) {
                                // NEW: Init stats
                                finalData.stats = { inventario: 0, rangoPrecios: [] };
                                // scoreCard is undefined initially
                            } else {
                                // EXISTING: Protect stats & scoreCard (calculated external)
                                delete finalData.stats;
                                delete finalData.scoreCard;
                            }
                        } catch (e) { logger.error(`Error Upsert Desarrollo ${finalData.id}`, e); }
                    }
                    ...
                    if (collectionName === 'desarrollos') {
                        ...
                        if (finalData.constructora) affectedConstructorNames.add(finalData.constructora); // Track dev name
                    } else if (collectionName === 'modelos') {
            ...
            // 7. Recalculate Developers (Triggered by Desarrollos or Modelos OR Desarrolladores)
            // Strategy: 
            // - If 'desarrolladores': use IDs directly.
            // - If 'desarrollos': use 'affectedConstructorNames' to find IDs.
            // - If 'modelos': use 'affectedDevelopmentIds' -> fetch devs -> get names -> find IDs.
            ...
            const resolveNamesToIds = async (names) => {
                ...
                // Firestore 'in' limit 10. Split tasks or just query one by one?
                // One buy one is safer for unlimited size
                for (const name of namesArray) {
                    const snap = await db.collection('desarrolladores').where('nombre', '==', name).limit(1).get();
                    if (!snap.empty) {
                        devsToRecalculate.add(snap.docs[0].id);
                    }
                }
            };

            if (collectionName === 'desarrollos' && affectedConstructorNames.size > 0) {
                await resolveNamesToIds(affectedConstructorNames);
            }

            if (collectionName === 'modelos' && affectedDevelopmentIds.size > 0) {
                // Fetch developments to find their constructors
                const devIds = Array.from(affectedDevelopmentIds);
                for (const devId of devIds) {
                    const snap = await db.collection('desarrollos').doc(devId).get();
                    if (snap.exists) {
                        const d = snap.data();
                        if (d.constructora) affectedConstructorNames.add(d.constructora);
                    }
                }
                await resolveNamesToIds(affectedConstructorNames);
            }
