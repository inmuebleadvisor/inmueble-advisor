...
export default function AccountAsesor() {
  ...
  const [selectedNewDevs, setSelectedNewDevs] = useState([]);
  ...
  const handleSaveInventory = async () => {
    if (selectedNewDevs.length === 0) return alert("Selecciona al menos un desarrollo.");
    setSavingInv(true);
    try {
      const nuevosItems = selectedNewDevs.map(id => ({
        tipo: 'db',
        idDesarrollo: id,
      ...
      // Actualizamos Firestore directo
      const docRef = doc(db, "users", user.uid);
      // Nota: arrayUnion no funciona tan simple con objetos si no son IDÉNTICOS, 
      // pero aquí estamos agregando nuevos. Para seguridad de no duplicar lógica compleja,
      // simplemente leemos el actual del perfil y agregamos. 
      // UserProfile ya lo tenemos en contexto pero puede estar stale, mejor confiar en arrayUnion 
      // O más simple: arrayUnion funciona por igualdad de objeto. 
      // Mejor usamos un merge manual simple con el userProfile actual que confiamos que es reciente.

      // Estrategia segura: arrayUnion de los nuevos objetos.
      await updateDoc(docRef, {
        inventario: arrayUnion(...nuevosItems)
      });

      alert("Solicitud enviada. Un administrador revisará tu petición.");
      setShowInvModal(false);
      setSelectedNewDevs([]);
      // Forzamos recarga de pagina o esperamos que el contexto se actualice si tuviera snapshot
      // Como UserContext usa getDoc, no se va a actualizar solo el widget lateral.
      // Haremos un reload suave o avisamos al usuario.
  ...
  return (
    <div className="main-content animate-fade-in" style={styles.container}>
        ...
        <aside style={styles.sideColumn}>
          ...
          <div style={styles.inventoryWidget}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '15px' }}>
              <h3 style={{ ...styles.widgetTitle, margin: 0 }}>Mi Inventario</h3>
              <button onClick={handleOpenInventory} style={styles.btnAddInv} title="Solicitar más desarrollos">+</button>
            </div>
      ...
      {/* MODAL SIMPLIFICADO DE INVENTARIO */}
      {showInvModal && (
        <div style={styles.overlay}>
          <div style={styles.modalInv}>
            <div style={styles.modalHeader}>
              <h3>Solicitar Nuevo Desarrollo</h3>
              <button onClick={() => setShowInvModal(false)} style={styles.closeBtn}>&times;</button>
            </div>
            <div style={styles.modalBody}>
              <p style={{ marginBottom: '15px', color: '#666', fontSize: '0.9rem' }}>Selecciona los desarrollos que deseas comercializar. Quedarán pendientes de aprobación.</p>
              <div style={styles.invGrid}>
                {allDevelopments.filter(d => !inventario.find(i => i.idDesarrollo === d.id)).map(dev => {
                  const isSelected = selectedNewDevs.includes(dev.id);
                  return (
                    <div
                      key={dev.id}
                      onClick={() => toggleSelectDev(dev.id)}
                      style={{
                        ...styles.devCard,
                    ...
                    >
                      ...
                      <br /><span style={{ fontSize: '0.8rem', color: '#888' }}>{dev.ubicacion?.ciudad}</span>
                    </div>
                  )
                })}
              </div>
              {allDevelopments.length > 0 && allDevelopments.filter(d => !inventario.find(i => i.idDesarrollo === d.id)).length === 0 && (
                <p style={{ textAlign: 'center', padding: '20px' }}>Ya tienes todos los desarrollos disponibles.</p>
              )}
            </div>
            <div style={styles.modalFooter}>
              <button onClick={handleSaveInventory} disabled={savingInv} style={styles.btnPrimary}>
                {savingInv ? 'Enviando...' : 'Enviar Solicitud'}
              </button>
            </div>
          </div>
        </div>
...
const styles = {
  ...
  // Estilos del Modal Inventario (Nuevos)
  btnAddInv: { width: '24px', height: '24px', borderRadius: '50%', border: '1px solid #cbd5e1', background: 'white', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#64748b' },
  overlay: { position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.5)', zIndex: 3000, display: 'flex', alignItems: 'center', justifyContent: 'center' },
  modalInv: { backgroundColor: 'white', borderRadius: '16px', width: '90%', maxWidth: '500px', maxHeight: '80vh', display: 'flex', flexDirection: 'column' },
  ...
  invGrid: { display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(130px, 1fr))', gap: '10px' },
  devCard: { padding: '10px', border: '1px solid #ddd', borderRadius: '8px', cursor: 'pointer', fontSize: '0.9rem', textAlign: 'center' },
  modalFooter: { padding: '15px', borderTop: '1px solid #eee', textAlign: 'right' },
  btnPrimary: { backgroundColor: 'var(--primary-color)', color: 'white', border: 'none', padding: '10px 20px', borderRadius: '8px', cursor: 'pointer', fontWeight: 'bold' }
};