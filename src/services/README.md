...
-   **`MetaService`**: (Frontend) Maneja la inicialización del Pixel, rastreo de eventos y generación de `event_id` para deduplicación híbrida (Pixel + CAPI). Trabaja en conjunto con las Cloud Functions especializadas (`onLeadIntentMETA`, `onLeadContactMETA`, `onLeadPageViewMETA`) para eventos de alta intención.
