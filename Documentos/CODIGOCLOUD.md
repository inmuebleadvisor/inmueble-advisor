### El codigo index de la google cloud es el siguiente:
...
exports.actualizarMetricasAsesor = onDocumentUpdated("leads/{leadId}", async (event) => {
  ...
  try {
    ...
    await db.runTransaction(async (t) => {
      ...
      // Se deja abierto según instrucción: "1 punto por cada 1.5 puntos porcentuales".

      t.update(asesorRef, {
        scoreGlobal: scoreTotal,
        "metricas.tasaCierre": Number(tasaCierre.toFixed(1)),
        "metricas.totalLeadsAsignados": total,
        "metricas.puntosCierre": puntosCierre, // Guardamos desglose para transparencia
